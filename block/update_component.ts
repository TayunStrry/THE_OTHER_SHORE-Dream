 ...
 * * < 方块 > 更新后 事件
 */
export default function BlockUpdateAfterEvent(block: server.Block) {
	/**
	 * * 执行 方块更新事件 的 方块列表
	 */
	...
	blocks.forEach(
		block => {
			switch (block.typeId) {
				case 'starry_map:enable_control': enableControl(block); break;

				case 'starry_map:basic_pipeline': basicPipeline(block); break;

				case 'starry_map:pulse_latch': pulseLatch(block); break;

				case 'starry_map:magic_portal_above': portalGate(block); break;

				case 'starry_map:magic_portal_below': portalGate(block); break;

				case 'starry_map:jungle_wood_chair': jungleWoodChair(block); break;

				case 'starry_map:diorite_table': dioriteTable(block); break;

				default: break;
			}
			if (block.hasTag('tags:machine_gate.horizontal_gate')) horizontalGate(block);
			if (block.hasTag('tags:machine_gate.vertical_gate')) verticalGate(block);
		}
	)
};
 ...
 */
function horizontalGate(object: server.Block) {
	/**
	 * * 比较方块对象
	 *
	 ...
	 * @returns - 是否为同一种方块
	 */
	function compareBlocks(target: server.Block | undefined) {
	...
	// 查询方块标签
	const [direction_0, direction_1, direction_2, direction_3]: boolean[] = [
...
function jungleWoodChair(object: server.Block) {
	if (object.permutation.getState('STATE:stage') != 0) {
		...
		switch (object.permutation.getState('minecraft:cardinal_direction')) {
			case 'south':
				if (object.east()?.hasTag('tags:home_decorating.chair.south')) opal.TrySetPermutation(object, 'STATE:stage', 1);
				else if (object.west()?.hasTag('tags:home_decorating.chair.south')) opal.TrySetPermutation(object, 'STATE:stage', 2);
				break;

			case 'north':
				if (object.east()?.hasTag('tags:home_decorating.chair.north')) opal.TrySetPermutation(object, 'STATE:stage', 2);
				else if (object.west()?.hasTag('tags:home_decorating.chair.north')) opal.TrySetPermutation(object, 'STATE:stage', 1);
				break;

			case 'east':
				if (object.south()?.hasTag('tags:home_decorating.chair.east')) opal.TrySetPermutation(object, 'STATE:stage', 2);
				else if (object.north()?.hasTag('tags:home_decorating.chair.east')) opal.TrySetPermutation(object, 'STATE:stage', 1);
				break;

			case 'west':
				if (object.south()?.hasTag('tags:home_decorating.chair.west')) opal.TrySetPermutation(object, 'STATE:stage', 1);
				else if (object.north()?.hasTag('tags:home_decorating.chair.west')) opal.TrySetPermutation(object, 'STATE:stage', 2);
				break;
...
function dioriteTable(object: server.Block) {
	if (object.permutation.getState('STATE:stage') != 0) {
		...
		switch (object.permutation.getState('minecraft:cardinal_direction')) {
			case 'south':
				if (object.east()?.hasTag('tags:home_decorating.table.south') && object.west()?.hasTag('tags:home_decorating.table.south')) opal.TrySetPermutation(object, 'STATE:stage', 3);
				else if (object.east()?.hasTag('tags:home_decorating.table.south')) opal.TrySetPermutation(object, 'STATE:stage', 2);
				else if (object.west()?.hasTag('tags:home_decorating.table.south')) opal.TrySetPermutation(object, 'STATE:stage', 1);
				break;

			case 'north':
				if (object.east()?.hasTag('tags:home_decorating.table.north') && object.west()?.hasTag('tags:home_decorating.table.north')) opal.TrySetPermutation(object, 'STATE:stage', 3);
				else if (object.east()?.hasTag('tags:home_decorating.table.north')) opal.TrySetPermutation(object, 'STATE:stage', 1);
				else if (object.west()?.hasTag('tags:home_decorating.table.north')) opal.TrySetPermutation(object, 'STATE:stage', 2);
				break;

			case 'east':
				if (object.south()?.hasTag('tags:home_decorating.table.east') && object.north()?.hasTag('tags:home_decorating.table.east')) opal.TrySetPermutation(object, 'STATE:stage', 3);
				else if (object.south()?.hasTag('tags:home_decorating.table.east')) opal.TrySetPermutation(object, 'STATE:stage', 1);
				else if (object.north()?.hasTag('tags:home_decorating.table.east')) opal.TrySetPermutation(object, 'STATE:stage', 2);
				break;

			case 'west':
				if (object.south()?.hasTag('tags:home_decorating.table.west') && object.north()?.hasTag('tags:home_decorating.table.west')) opal.TrySetPermutation(object, 'STATE:stage', 3);
				else if (object.south()?.hasTag('tags:home_decorating.table.west')) opal.TrySetPermutation(object, 'STATE:stage', 2);
				else if (object.north()?.hasTag('tags:home_decorating.table.west')) opal.TrySetPermutation(object, 'STATE:stage', 1);
				break;
