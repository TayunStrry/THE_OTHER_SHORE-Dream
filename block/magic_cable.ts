 ...
 */
function cableQueue(block: server.Block, type: string): server.Block[] {
	/**
	 ** 设定方块的原始数组
	 */
 ...
 */
function tryBatchState(queue: server.Block[], type: string, value: string | number | boolean): Error | void {
	try {
		queue.forEach(
			block => {
				/**
 ...
 */
function redstoneChange(object: server.Block, states: server.BlockPermutation): boolean {
	try {
		/**
		 ** 获取方块的红石能量
		 */
...
export function redstoneDetection(object: server.Block, type: string) {
	if (!redstoneChange(object, object.permutation)) tryBatchState(cableQueue(object, type), 'STATE:rune_type', opal.Random({ max: 7, min: 1 }, true));
};
 ...
 */
function runDetection(self: server.Block, source: server.Vector3): server.Block | undefined {
	// 遍历 32 个方块
...
export function signalCompilation(object: server.Block, type: string, states: server.BlockPermutation) {
	...
	switch (getContainer.getItem(getSlot)?.typeId) {
		case 'starry_map:blue_energy': tryBatchState(blocks, 'STATE:rune_type', 1); break;

		case 'starry_map:red_energy': tryBatchState(blocks, 'STATE:rune_type', 2); break;

		case 'starry_map:green_energy': tryBatchState(blocks, 'STATE:rune_type', 3); break;

		case 'starry_map:orange_energy': tryBatchState(blocks, 'STATE:rune_type', 4); break;

		case 'starry_map:purple_energy': tryBatchState(blocks, 'STATE:rune_type', 5); break;

		case '能量水晶:启程_魔晶石': tryBatchState(blocks, 'STATE:rune_type', 6); break;

		case '能量水晶:焚绝_魔晶石': tryBatchState(blocks, 'STATE:rune_type', 7); break;

		default: tryBatchState(blocks, 'STATE:rune_type', 0); break;
	};
	// 修改 物品容器 索引
...
export function interactiveTerminal(object: server.Block, type: string, states: server.BlockPermutation) {
	const blocks = cableQueue(object, type);
	tryBatchState(blocks, 'STATE:rune_type', states.getState('STATE:rune_note') as number);
};
...
export function defaultEvent(object: server.Block, type: string, state: server.BlockPermutation) {
	...
	const blocks = cableQueue(object, type);
	// 遍历 方块队列
	tryBatchState(blocks, 'STATE:rune_type', state.getState('STATE:rune_type') as number);
	opal.TrySetPermutation(object, 'STATE:rune_type', 9);
};