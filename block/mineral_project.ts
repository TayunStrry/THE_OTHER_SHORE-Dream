 ...
 ** 初级矿井单元 < 150 能量消耗 >
 */
export function Mine(object: server.Block) {
	// 判断能量值 是否足够
	if (!opal.ExpendEnergy(object, -150)) return;
	/**
	 ** 当前节点名称
	 */
	...
	// 如果 区域属性名称 为空
	if (!realmName) return opal.CreateMineralVein(current);
	/**
	 ** 获取 方块容器
	 */
	...
	// 判断 物品容器 是否存在
	if (!container || container.emptySlotsCount == 0) opal.TrySpawnItem(object.dimension, itemStack, opal.Vector.add(object, { x: 0.5, y: 1, z: 0.5 }))
