...
components.set(componentPrefix + 'jungle_wood_chair',
	{
		async onPlayerInteract(source: server.BlockComponentPlayerInteractEvent) {
			...
			// 创建 玩家座位点实体
			opal.TrySpawnEntity(analysis.dimension, 'starry_map:execute.player_seat', analysis.block.center() as server.Vector3);
		}
	}
);
/*
 ...
 */
function injectContainers(block: server.Block, item: server.ItemStack, container: server.Container, slot: number, params: ReturnType<typeof opal.SearchContainers>) {
	/**
	 * * 获取目标容器和目标方块
	 */
...
components.set(componentPrefix + 'document_display',
	{
		onPlayerInteract(source: server.BlockComponentPlayerInteractEvent, data: { params: DOCUMENT_DISPLAY }) {
			/**
...
components.set(componentPrefix + 'diffusion_filling',
	{
		onPlayerInteract(source: server.BlockComponentPlayerInteractEvent, data: { params: DIFFUSION_FILLING }) {
			//todo 事件预处理
			/**
			 * 解构交互事件对象, 获取关键参数：
			 *
...
components.set(componentPrefix + 'state_value_increase',
	{
		onPlayerInteract(source: server.BlockComponentPlayerInteractEvent, data: { params: STATE_VALUE_INCREASE }) {
			/**
			 * 解构交互事件对象, 获取关键参数：
			 *
			 * - player: 触发事件的玩家对象
			 ...
			 * 获取需要修改的属性的当前值
			 */
			const value = state.getState(type) as number;
			/**
			 * 获取属性值修改结果
			 */
			const result = opal.TrySetPermutation(block, type, value + 1);
			// 如果修改失败, 则重置属性值
			if (result instanceof Error) opal.TrySetPermutation(block, type, 0);
			// 显示提示信息
