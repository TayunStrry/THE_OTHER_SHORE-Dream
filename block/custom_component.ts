...
class BlockSpreadSystem {
	 ...
	 */
	static createSpreadProcessor(block: server.Block, state: server.BlockPermutation, directions: opal.Vector[], player?: server.Player, callback?: (currentBlock: server.Block, adjacentBlock: server.Block) => void) {
		// 初始化待处理方块队列，从起始方块开始
		...
		const executeSpreadQuery = () => {
			...
			for (let index = 0; index < 1024 && keepRun; index++) {
				...
				if (blocksToMine.length === 0 || minedCount >= 10240 || !keepRun) return stopEvent();
				/** 从队列中取出当前处理的方块 */
			 ...
			 */
			function executeBlockUpdate(currentBlock: server.Block) {
				try {
					// 遍历所有指定的方向
					for (const direction of directions) {
						...
						if (!adjacentBlock || adjacentBlock.typeId !== block.typeId || adjacentRuneState !== state.getState('STATE:rune_type')) continue;
						// 如果有自定义回调则使用回调，否则使用默认状态切换逻辑
						if (callback) callback(currentBlock, adjacentBlock);
						else {
							/** 获取相邻方块的当前符文类型状态 */
	 ...
	 */
	static toggleRuneState(block: server.Block, currentState: number, targetState: string | number | boolean | undefined = 7) {
...
components.set(prefix[1] + 'vertical_gate',
	{
		onPlayerInteract(source: server.BlockComponentPlayerInteractEvent) {
			...
			if (item?.typeId == block.typeId) {
				...
				else player?.playSound('block.stonecutter.use');
				// 终止函数的后续运行
				return;
			}
			// 阻断玩家过于频繁的交互
			if (!player || !opal.TriggerControl('玩家与机关门进行交互', player, 10)) return;
			// 启动方块状态传播（全方向）
			BlockSpreadSystem.createSpreadProcessor(block, state, opal.Vector.CONSTANT_ALL, player);
			// 立即切换当前方块状态
			BlockSpreadSystem.toggleRuneState(block, blockRuneState as number);
			// 播放关闭音效
...
components.set(prefix[4] + 'bus_port',
	{
		onTick(source: server.BlockComponentTickEvent) {
			...
			if (condition === 0 || condition === 9) {
				// 如果状态为9，重置为0
				if (condition === 9) opal.TrySetPermutation(block, 'STATE:rune_type', 0);
				// 终止后续逻辑执行
				return;
			}
			/** 定义朝向与坐标轴的映射关系 */
			const faceToAxisMap: Record<string, server.Vector3[]> = {
			...
			/** 根据朝向获取对应的坐标轴方向 */
			const axis = faceToAxisMap[state.getState('minecraft:block_face') as string];
			// 检查是否存在有效朝向
			if (!axis) return;
			// 遍历两端相邻方块
			axis.map(axis => block.offset(axis)).forEach(
				contactBlocks => {
					// 跳过无效方块或状态为9的方块
					if (!contactBlocks || contactBlocks.permutation.getState('STATE:rune_type') == 9) return;
					// 如果是另一个总线端口，则直接设置状态
					if (contactBlocks.typeId == block.typeId) return opal.TrySetPermutation(contactBlocks, 'STATE:rune_type', condition);
					// 如果是机关门方块或者其他魔导总线方块，则启动状态传播
					if (contactBlocks.hasTag('tags:machine_gate.series') || contactBlocks.hasTag('tags:magic_cable.series')) {
						// 使用自定义回调处理状态切换
						BlockSpreadSystem.createSpreadProcessor(contactBlocks, contactBlocks.permutation, opal.Vector.CONSTANT_ALL, undefined,
							(_, adjacentBlock) => {
								/** 相邻方块的符文类型状态 */
								const adjacentState = adjacentBlock.permutation.getState('STATE:rune_type');
								// 根据条件切换状态：0变为条件值，非0变为0
								BlockSpreadSystem.toggleRuneState(adjacentBlock, adjacentState as number, condition);
							}
						);
						// 切换当前接触方块的状态
						BlockSpreadSystem.toggleRuneState(block, state.getState('STATE:rune_type') as number, condition);
						// 如果是机关门则播放关闭音效
						if (contactBlocks.hasTag('tags:machine_gate.series')) block.dimension.playSound('close.iron_door', block.bottomCenter());
					}
				}
			);
			// 设置总线端口自身状态为9（激活状态）
...
components.set(prefix[1] + 'diffusion_filling',
	{
		onPlayerInteract(source: server.BlockComponentPlayerInteractEvent, data: server.CustomComponentParameters) {
			...
			try {
				 ...
				 * @type {number}
				 */
				const magnification: number = 1024;
				/**
				 * 存储每 tick 的 ID
				 */
				const tickIds: number[] = [];
...
components.set(prefix[4] + 'servo_drive',
	{
		onTick(source: server.BlockComponentTickEvent) {
			...
			const analysis = customFunction.TickComponentTrigger(source);
			// 如果未完成前处理则执行前处理事件
			if (analysis.condition !== 0) customFunction.servoDriveBeforeEvent(analysis, -1);
			// 否则执行后处理事件
			else if (analysis.state.getState('STATE:direction') !== 0) customFunction.servoDriveAfterEvent(analysis, 1,
				(dimension, anchor_0, anchor_1, anchor_2, anchor_3) => {
