 ...
 * @param source - 方块组件参数
 */
export function InteractComponentTrigger(source: server.BlockComponentPlayerInteractEvent): customType.INTERACT_COMPONENT {
    /**
     * * 方块对象
     */
 ...
 */
export function blockTimer(block: server.Block, bounds: number, after: (block: server.Block) => void): Error | void {
    /**
     ** 方块位置
     */
 ...
 * @param source - 方块组件参数
 */
export function TickComponentTrigger(source: server.BlockComponentTickEvent): customType.TICK_COMPONENT {
    /**
     * * 方块对象
     */
    const block = source.block;
...
export function AboveTeleport(block: server.Block) {
    ...
    // 执行事件机制
    if (!opal.VisualizeUseStardustEnergy(block, -30)) return;
    // 遍历 射线方向 的 128个 方块
 ...
 */
function resolveMagicCableNeighborsByType(block: server.Block, type: string): server.Block[] {
    /**
     ** 设定方块的原始数组
     */
    ...
    try {
        ...
        switch (getPoint[0]) {
            case 'Xx':
                if (portPositive_X?.hasTag("tags:magic_cable.port_negative.X")) getProtoArray.push(portPositive_X);
                if (portNegative_X?.hasTag("tags:magic_cable.port_positive.X")) getProtoArray.push(portNegative_X);
                break;
        ...
        switch (getPoint[1]) {
            case 'Yy':
                if (portPositive_Y?.hasTag("tags:magic_cable.port_negative.Y")) getProtoArray.push(portPositive_Y);
                if (portNegative_Y?.hasTag("tags:magic_cable.port_positive.Y")) getProtoArray.push(portNegative_Y);
                break;
        ...
        switch (getPoint[2]) {
            case 'Zz':
                if (portPositive_Z?.hasTag("tags:magic_cable.port_negative.Z")) getProtoArray.push(portPositive_Z);
                if (portNegative_Z?.hasTag("tags:magic_cable.port_positive.Z")) getProtoArray.push(portNegative_Z);
                break;
 ...
 */
export function redstoneDetection(block: server.Block, type: string, currentPermutation: server.BlockPermutation): Error | void {
    try {
        /**
         * 获取上方红石信号强度
         */
         ...
         * 获取当前保存的红石值
         */
        const storedPower = currentPermutation.getState('STATE:redstone') as number;
        // 如果无红石信号则重置状态并返回
        if (redstonePower == undefined) return opal.TrySetPermutation(block, 'STATE:redstone', 0);
        // 如果红石能量值发生变化，则更新方块状态
 ...
 */
export function superPulse(self: server.Block, type: string): server.Block | undefined {
    try {
         ...
         * 定义朝向映射
         */
        const directionMap: Record<string, server.Vector3> = {
        ...
        for (let index = 1; index < 32; index++) {
             ...
             * 获取目标方块的状态值
             */
            const state = (target?.permutation.getState('STATE:rune_type') as number) ?? 9;
            // 检查是否为目标魔导总线方块
            if (!target?.hasTag('tags:magic_cable.series') || state !== 0 || target.typeId === self.typeId) continue;
            // 激活目标方块
            opal.TrySetPermutation(target, 'STATE:rune_type', self.permutation.getState('STATE:rune_type') as number);
            // 返回被选中的方块
            return target;
        }
...
export function superOmphalos(object: server.Block, states: server.BlockPermutation) {
    ...
    try {
        ['up', 'down', 'north', 'south', 'west', 'east'].forEach(
            // 替换指定方向的方块
            info => opal.TryFillBlocks(
                object.dimension,
                opal.Vector.add(object, { x: 11, y: 11, z: 11 }),
                opal.Vector.add(object, { x: -11, y: -11, z: -11 }),
...
export function interactiveTerminal(object: server.Block, type: string, states: server.BlockPermutation) {
    const blocks = resolveMagicCableNeighborsByType(object, type);
    opal.TrySetPermutation(blocks, 'STATE:rune_type', states.getState('STATE:rune_note') as number);
};
...
export function logicComponents(object: server.Block, type: string) {
    ...
    if (object.typeId == 'starry_map:logic_inverter') return;
    // 重置逻辑锁存器
    for (let axleX = -1; axleX <= 1; axleX++) for (let axleY = -1; axleY <= 1; axleY++) for (let axleZ = -1; axleZ <= 1; axleZ++) {
        ...
        if (block?.typeId == 'starry_map:pulse_latch') opal.TrySetPermutation(block, 'STATE:rune_note', 0);
    };
};
/**
 ** 信号处理
...
function logicSingle(object: server.Block) {
    ...
    function detection_up() {
        ...
        if (!input || !input?.hasTag('tags:magic_cable.open')) return;
        if (!output || !output?.hasTag('tags:magic_cable.series')) return;
        /**
         ** 方块属性
         */
        const states = input.permutation;
        opal.TrySetPermutation(output, 'STATE:rune_type', states.getState('STATE:rune_note') as number);
 ...
 */
export function LatchUpdateEvent(block: server.Block) {
    /**
     ** 执行 方块更新事件 的 方块列表
     */
    ...
    blocks.forEach(
        block => {
            switch (block.typeId) {
                case 'starry_map:logic_single': logicSingle(block); break;

                case 'starry_map:logical_and': opal.TrySetPermutation(block, 'STATE:stage', 1); break;

                case 'starry_map:exclusive_or': opal.TrySetPermutation(block, 'STATE:stage', 1); break;
 ...
 */
export function activateConnectedMagicCables(object: server.Block, type: string, state: server.BlockPermutation) {
    /**
     ** 获取 方块队列
     */
    const blocks = resolveMagicCableNeighborsByType(object, type);
    // 遍历设置与之连接的方块状态
    opal.TrySetPermutation(blocks, 'STATE:rune_type', state.getState('STATE:rune_type') as number);
    // 设置自身的方块状态
    opal.TrySetPermutation(object, 'STATE:rune_type', 9);
};
/**
 * * 物资收集 < 30 能量消耗 >
 */
export function materialCollection(block: server.Block, permutation: server.BlockPermutation) {
    // 判断能量值 是否足够
    if (!opal.VisualizeUseStardustEnergy(block, -30)) return;
    /**
     * * 实体查询选项
 ...
 * * 方块放置 < 60 能量消耗 >
 */
export function blockPlacement(block: server.Block) {
    // 判断能量值 是否足够
    if (!opal.VisualizeUseStardustEnergy(block, -60)) return;
    /**
     * * 获取上方的方块对象
     */
 ...
 * * 打包投送 < 60 能量消耗 >
 */
export function packageDelivery(block: server.Block, type: string) {
    // 判断能量值 是否足够
    if (!opal.VisualizeUseStardustEnergy(block, -60)) return;
    /**
     * * 定义 物品信息 的 缓存数组
 ...
 * * 驱动核心 < 120 能量消耗 >
 */
export function servoOmphalos(object: server.Block, type: string) {
    // 判断能量值 是否足够
    if (!opal.ControlStardustEnergy(object, -120)[1]) return;
    /**
     * * 结构范围 上极限 坐标
     */
     ...
     * * 设置 方块执行 的 指令
     */
    const RunCode = (info: string) => object.dimension.runCommand(info);
    /**
     * * 移动 范围内 的 全部实体
     *
     * @param {server.Vector3} args 移动 方向
     */
    function MoveEntity(args: server.Vector3) {
        try {
            /**
             * * 设置 实体查询选项
             */
    ...
    for (let index = 0; index <= 10; index++) {
        if (rangeMax.x == object.x + 10) {
            ...
            // 判断 方块 是否 符合要求
            if (getBlock && getBlock.isAir) rangeMax.x = object.x + index;
        }
        if (rangeMin.x == object.x - 10) {
            ...
            const getBlock = object.offset({ x: -index, y: 0, z: 0 });
            // 判断 方块 是否 符合要求
            if (getBlock && getBlock.isAir) rangeMin.x = object.x - index;
        }
        if (rangeMax.y == object.y + 10) {
            ...
            // 判断 方块 是否 符合要求
            if (getBlock && getBlock.isAir) rangeMax.y = object.y + index;
        }
        if (rangeMin.y == object.y - 10) {
            ...
            const getBlock = object.offset({ x: 0, y: -index, z: 0 });
            // 判断 方块 是否 符合要求
            if (getBlock && getBlock.isAir) rangeMin.y = object.y - index;
        }
        if (rangeMax.z == object.z + 10) {
            ...
            // 判断 方块 是否 符合要求
            if (getBlock && getBlock.isAir) rangeMax.z = object.z + index;
        }
        if (rangeMin.z == object.z - 10) {
            ...
            const getBlock = object.offset({ x: 0, y: 0, z: -index });
            // 判断 方块 是否 符合要求
            if (getBlock && getBlock.isAir) rangeMin.z = object.z - index;
        }
    };
    //将区块内的方块移动到指定位置
    switch (type) {
...
export function servoDriveBeforeEvent(analysis: customType.TICK_COMPONENT, offsetY: number = -1) {
    ...
    if (condition >= 1 && condition <= 4) {
        ...
        if (hasTag(config.offset, 'tags:magic_cable.series') && !hasTag(config.checkOffset, 'tags:magic_cable.series')) {
            // 判断能量值 是否足够
            if (!opal.ControlStardustEnergy(block, -30)[1]) return;
            // 执行 伺服基座 移动许可
 ...
 ** 初级造石单元 < 35 能量消耗 >
 */
export function Solidify(object: server.Block) {
    // 判断能量值 是否足够
    if (!opal.VisualizeUseStardustEnergy(object, -35)) return;
    /**
     ** 石材类型
     */
    ...
    // 判断 物品容器 是否存在
    if (!container || container.emptySlotsCount == 0) opal.TrySpawnItem(object.dimension, item, opal.Vector.add(object, { x: 0.5, y: 1, z: 0.5 }))
    else container.addItem(item);
};
/**
 ** 破坏核心 < 15 能量消耗 >
 */
export function Destroy(object: server.Block, type: string) {
    // 判断能量值 是否足够
    if (!opal.VisualizeUseStardustEnergy(object, -15)) return;
    /**
     ** 坐标组
     */
    const locationGroup: server.Vector3[] = [];
    // 获取 方块的 坐标
    switch (type) {
        case 'east': locationGroup.push(opal.Vector.add(object, opal.Vector.CONSTANT_EAST), opal.Vector.add(object, { x: 6, y: 0, z: 0 }));
            break;
        case 'west': locationGroup.push(opal.Vector.add(object, opal.Vector.CONSTANT_WEST), opal.Vector.add(object, { x: -6, y: 0, z: 0 }));
            break;
        case 'up': locationGroup.push(opal.Vector.add(object, opal.Vector.CONSTANT_UP), opal.Vector.add(object, { x: 0, y: 6, z: 0 }));
            break;
        case 'down': locationGroup.push(opal.Vector.add(object, opal.Vector.CONSTANT_DOWN), opal.Vector.add(object, { x: 0, y: -6, z: 0 }));
            break;
        case 'south': locationGroup.push(opal.Vector.add(object, opal.Vector.CONSTANT_SOUTH), opal.Vector.add(object, { x: 0, y: 0, z: 6 }));
            break;
        case 'north': locationGroup.push(opal.Vector.add(object, opal.Vector.CONSTANT_NORTH), opal.Vector.add(object, { x: 0, y: 0, z: -6 }));
            break;
    };
    /**
     ** 在路径上挖掘
 ...
 ** 金属锻压 < 15 能量消耗 >
 */
export function Forming(object: server.Block) {
    // 判断能量值 是否足够
    if (!opal.VisualizeUseStardustEnergy(object, -15)) return;
    /**
     ** 获取 方块容器
    ...
    // 判断 物品容器 是否存在
    if (!container || container.emptySlotsCount == 0) return opal.ErrorMessage('<§l§b 金属锻压 §r>§4 发生错误§r', object, { text: '未能在设备上方找到合适的<§l§3 方块容器 §r>' });
    // 遍历物品容器
    for (let index = 0; index < container.size; index++) {
        /**
         ** 获取 物品对象
 ...
 */
export function AllocationPowerInDirection(block: server.Block, offset: server.Vector3) {
    // 遍历 动能分配模块
 ...
 */
export function hasAdjacentAllocationPower(block: server.Block): boolean {
    /**
     * * 获取 附近的方块
     */
 ...
 */
export function distributeLavaToStorageTanks(object: server.Block) {
    try {
        /**
         * * 获取 自身 的 方块状态
         */
         ...
         * * 获取 熔岩储备
         */
        const getCacheValue = getPermutation.getState('STATE:magma') as number
        /**
         * * 获取 石材储备
         */
        const getStoneValue = getPermutation.getState('STATE:material') as number
        /**
         * * 获取 方块 的 原始数组
         */
        const getProtoArray: server.Block[] = [];
        ...
        // 确认方块是否存在
        if (portPositive_X) getProtoArray.push(portPositive_X);
        if (portNegative_X) getProtoArray.push(portNegative_X);
        if (portPositive_Z) getProtoArray.push(portPositive_Z);
        if (portNegative_Z) getProtoArray.push(portNegative_Z);
        /**
         * * 获取 过滤后 的 方块 的 数组
         */
        ...
        getBlockArray.forEach(
            info => {
                 ...
                 * * 获取 储罐 的 属性值
                 */
                const getInfoValue = getInfoStates.getState('STATE:magma') as number;
                // 储罐 未满时 进行 补充
                if (getInfoValue > 13) return
        ...
        for (let index = 0; index < container.size; index++) {
            ...
            const item = container.getItem(index);
            // 判断 物品 是否符合要求
            if (!item || item.amount < consume || item.typeId != 'starry_map:compressed_stone') continue;
            // 减少 物品堆栈
 ...
 * @param {server.Block} object - 执行事件的方块
 */
export function consumeEnergyAndAdvanceStage(object: server.Block) {
    // 判断能量值 是否足够
    if (!opal.VisualizeUseStardustEnergy(object, -45)) return;
    /**
     * * 获取 自身 的 方块状态
     */
    const getPermutation = object.permutation;
 ...
 */
export function extractResidues(container: server.Container, expense: string[]): boolean {
    /**
     * 有效的怪物掉落物类型集合
     */
