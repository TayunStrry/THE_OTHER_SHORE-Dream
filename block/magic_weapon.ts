 ...
 */
export function pulsePeakCannon(object: server.Block) {
	// 判断能量值 是否足够
	if (!opal.ExpendEnergy(object, -1)) return;
	/**
	 * * 设置 范围查询 的 过滤条条件
	 */
	 ...
	 * * 获取 目标列表
	 */
	const targets = object.dimension.getEntities(options).filter(
		entity => {
	...
	if (targets.length === 0 || !opal.ExpendEnergy(object, -150)) return;
	/**
	 * * 暴击概率
	 */
