...
import { globalState, CombatAttribute, componentPrefix } from "./state";
...
import { updateSoldiersBehavior } from "./legion";

/**
 * 确保军团基地生成点的唯一性
 */
export function registerEntitySpawnHandler(): void {
 ...
 */
export function registerWorldInitializeHandler(): void {
 ...
 * 监听原版按钮的点击事件
 */
export function registerButtonPushHandler(): void {
 ...
 * 监听实体触发器事件
 */
export function registerDataDrivenEntityTriggerHandler(): void {
...
export function registerItemComponentHandler(): void {
    server.world.beforeEvents.worldInitialize.subscribe(
        data => {
            ...
            data.itemComponentRegistry.registerCustomComponent(
                `${componentPrefix}set_public_property`,
                {
                    onUse: (event) => {
                        const { source } = event;
                        if (source instanceof server.Player) {
 ...
 * 监听实体生命值变化事件
 */
export function registerEntityHealthChangedHandler(): void {
 ...
 */
export function HealthAlterDisplay(entity: server.Entity, variation: number): void {
