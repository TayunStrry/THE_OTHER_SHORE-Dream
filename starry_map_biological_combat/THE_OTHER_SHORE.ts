...
itemComponents.set(componentPrefix + 'red_legion_base_config',
	{
		'onUse'(arg0) {
			// 如果玩家处于潜行状态，则修改健康值
			if (arg0.source.isSneaking) return setRedLegionBaseHealth(arg0.source);
			/**
			 * 表单窗口
			 */
			 ...
			 * 实体类型列表
			 */
			const keys: string[] = [...redCampRecruitmentList.keys()];
			/**
			 * 实体最大数量列表
			 */
			const amount: number[] = [...redCampRecruitmentList.values()].map(value => value.maxAmount);
			// 添加滑动条
			// @ts-expect-error
				...
				option => {
					...
					option.formValues.forEach(
						(value, index) => {
							...
							// 如果数据不存在或数据类型错误，则返回
							if (protoState == undefined || typeof value !== 'number') return;
							// 更新数据
...
function setRedLegionBaseHealth(player: server.Player) {
	/**
	 * 获取军团基地的实体数组
	 */
	const legionBases = player.dimension.getEntities({ type: 'red_legion:legion_base' });
	// 如果不存在则返回
	if (legionBases.length === 0) return;
	/**
	 * 表单窗口
	 */
	 ...
	 * 军团基地的健康值组件数组
	 */
	const healthComponents: server.EntityHealthComponent[] = [];
	/**
	 * 军团基地的实体数组
	 */
	const baseEntities: server.Entity[] = [];
	// 遍历军团基地
	legionBases.forEach(
		(entity, index) => {
			/**
		...
		option => {
			...
			option.formValues.forEach(
				async (value, index) => {
					...
					else for (let index = 0; index < healthComponents[index].currentValue; index++) {
						// 造成伤害
...
itemComponents.set(componentPrefix + 'blue_legion_base_config',
	{
		'onUse'(arg0) {
			// 如果玩家处于潜行状态，则修改健康值
			if (arg0.source.isSneaking) return setBlueLegionBaseHealth(arg0.source);
			/**
			 * 表单窗口
			 */
			 ...
			 * 实体类型列表
			 */
			const keys: string[] = [...blueCampRecruitmentList.keys()];
			/**
			 * 实体最大数量列表
			 */
			const amount: number[] = [...blueCampRecruitmentList.values()].map(value => value.maxAmount);
			// 添加滑动条
			// @ts-expect-error
			keys.forEach((key, index) => display.slider(key, 1, 100, 1, amount[index]));
			// 显示窗体
 ...
 * 设置蓝色军团基地健康值
 */
function setBlueLegionBaseHealth(player: server.Player) {
	/**
	 * 获取军团基地的实体数组
	 */
...
server.world.afterEvents.dataDrivenEntityTrigger.subscribe(
	event => {
		...
		switch (eventId) {
			case 'entity_event:red_legion_recruits_members': redLegionRecruitsMembers(entity); break;

			case 'entity_event:blue_legion_recruits_members': blueLegionRecruitsMembers(entity); break;
...
function redLegionRecruitsMembers(entity: server.Entity) {
	/**
	 * 获取维度与坐标
	 */
	...
	if (amount < state.maxAmount) {
		...
		// 设置实体名称
		if (target instanceof server.Entity) target.nameTag = "§4§o§l<| 红色军团 |>§r"
	}
};
function blueLegionRecruitsMembers(entity: server.Entity) {
	/**
	 * 获取维度与坐标
	 */
	const { dimension, location } = entity;
	...
	if (amount < state.maxAmount) {
		...
		const target = TrySpawnEntity(dimension, state.type, above);
		// 设置实体名称
		if (target instanceof server.Entity) target.nameTag = "§9§o§l<| 蓝色军团 |>§r"
	}
};
...
server.world.afterEvents.entityDie.subscribe(
	data => {
		...
		// 验证实体是否有效
		if (!validName.has(deadEntity.typeId) || !dimension) return;
		// 消耗现存的军团
