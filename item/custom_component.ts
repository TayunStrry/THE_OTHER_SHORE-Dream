...
components.set(componentPrefix + 'iridescent_prism',
    {
        'onHitEntity'(source: server.ItemComponentHitEntityEvent, data: server.CustomComponentParameters) {
            /**
             * 获取自定义组件的参数
             */
            const params = data.params as customType.IRIDESCENT_PRISM;
            /**
             * 对目标施加的状态效果
             */
...
components.set(componentPrefix + 'use_effect',
    {
        onConsume(source: server.ItemComponentConsumeEvent, data: server.CustomComponentParameters) {
            /**
             * * 获取自定义参数
             */
            const params = data.params as customType.USE_EFFECT;
            /**
             * 获取当前使用者
             */
...
components.set(componentPrefix + 'bee_fire_cuisine',
    {
        onConsume(data) {
            ...
            if (damageIncrease <= healthValue * 10) opal.AlterProperty(creature, { 'damage_increase': healthValue * 2, 'self_rune': 'rune_red' });
            // 根据符文颜色设置粒子颜色
...
components.set(componentPrefix + 'general_food_effects_abyssal_whale',
    {
        async onConsume(data) {
            ...
            // 在当前位置范围内寻找空气并替换为水
            opal.TryProcessBlocksInVolume(creature.dimension, location, 4, { 'includeTypes': ['minecraft:air'] }, (block: server.Block) => block.setType('minecraft:flowing_water'));
            // 等待60个游戏刻
            await server.system.waitTicks(60);
            // 在当前位置范围内寻找水并替换为空气
            opal.TryProcessBlocksInVolume(creature.dimension, location, 8, { 'includeTypes': ['minecraft:flowing_water', 'minecraft:water'] }, (block: server.Block) => block.setType('minecraft:air'));
        }
    }
);
/*
...
components.set(componentPrefix + 'space_transition',
    {
        onUse(source) {
            ...
            else customFunction.basePresets().show(player).then(
                option => {
                    if (option.canceled) return;
                    switch (option.selection) {
                        //相对传送
                        case 0: customFunction.relativePresets().show(player).then(option => customFunction.renRelative(player, option)); break;

                        //随机传送
                        case 1: customFunction.randomPresets().show(player).then(option => customFunction.renRandom(player, option)); break;

                        //诸界道标
                        case 2: customFunction.signPresets(RoadSign).show(player).then(option => customFunction.renRoadSign(player, option, RoadSign)); break;
                    }
                }
            );
            // 更新 物品冷却
            ...
            display.show(player).then(
                result => {
                    ...
                    switch (result.selection) {
                        // 绑定模式
                        case 0: customFunction.bindingAnchor(player); break;
                        // 召集模式
                        case 1: customFunction.musterAnchor(player); break;
                        // 移除模式
                        case 2: customFunction.deleteAnchor(player); break;
                        // 传送模式
                        case 3: customFunction.arrivalAnchor(player); break;
                    };
                    // 播放 音效
...
components.set(componentPrefix + 'magic_crystal_bow',
    {
        onUse(source) {
            ...
            if (!opal.TriggerControl(item.typeId, player, 10)) return;
            // 执行 弹弓照明 机制
            if (player.isSneaking) customFunction.bowSneaking(player)
            // 如果 玩家 不在潜行模式
            else {
                /**
                 * * 获取 玩家属性面板
...
components.set(componentPrefix + 'magic_crystal_screwdriver',
    {
        ...
        onMineBlock(source) {
            ...
            // 条件检查
            if (!(player instanceof server.Player) || !customFunction.canTriggerChainMining(player, itemStack, type) || !itemStack) return;
            /**
             * 获取被挖掘的方块标识符
             */
            ...
            // 检查耐久度是否足够
            if (!customFunction.checkDurability(durability, waste, player, itemStack)) return;
            // 应用挖掘后状态效果
            ...
            // 更新物品状态
            if (container) container.setItem(player.selectedSlotIndex, itemStack);
        }
    }
);
/*
...
components.set(componentPrefix + 'chorus_picture',
    {
        onUse(source) {
            ...
            // 执行初始化生成
            if (!rule) return customFunction.initialSummoning(player);
            // 召唤 随机角色
