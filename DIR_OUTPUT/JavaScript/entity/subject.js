...
export function HealthAlterDisplay(entity, variation) {
    ...
    if (!entity || !entity.isValid || !opal.TriggerControl('生命值变动 -> ' + variation, entity))
        return;
    /**
     ...
     * @type {number}
     */
    const baseOffset = table.offset_show.get(entity.typeId) ?? 1.5;
    /**
     * 生成随机的显示位置偏移量, 用于在实体上方显示粒子效果时增加随机性
     *
     * @type {server.Vector3}
     */
    const randomOffset = opal.Vector.random(opal.Vector.CONSTANT_ZERO, baseOffset, opal.Vector.CONSTANT_UP);
     ...
     */
    const entityData = opal.GetProperty(entity);
    /**
     ...
     * @type {server.RGB}
     */
    const color = table.getRuneColor(entityData.add_rune);
    /**
 ...
 */
export function EnterVacantSpaceWaspTower(player) {
    /**
     * * 获取 游戏规则
     */
    ...
    // 检测 建筑结构
    if (!template)
        return player.sendMessage([opal.translate(player), { text: '-> 未能获取到<§l§9 末地蜂塔 §r>的结构数据文件' }]);
    // 放置 建筑结构
 ...
 */
export function GenerateOnSchedule(type, time, title, sound) {
    /**
     * * 日历计时值
     */
 ...
 */
export function BlockChainEvent(eventData, type) {
    /**
     * * 被挖掘的方块的标识符
     */
    ...
    // 判断 耐久度是否足够
    if (waste >= surplus)
        return player.sendMessage([opal.translate(item), { text: ' -> 耐久度不足, 无法连锁' }]);
    // 为 玩家 添加状态效果
    ...
    // 创建 路径执行计划
    opal.PathExecute.CreateForCube('区块连锁-路径执行', {
 ...
 */
export function ReviseWorldRules(dimension) {
    // 禁止发送命令反馈
 ...
 */
export function EntityUnderAttack(target, source, entity, damage) {
    // 检测是否为 玩家类型
    ...
    switch (target.typeId) {
        case 'minecraft:ender_dragon':
            passive_component.EnhanceEnderDragon(target, entity, damage, source.cause);
            break;
        case 'starry_map:wild_bee.emperor':
            passive_component.WaspReinforce(target);
            break;
        case 'starry_map:guide.jasmine':
            passive_component.PrayerOfHundredFlowers(target);
            break;
        case 'starry_map:abyss_whale.emperor':
            passive_component.AbysssWhaleEmperorDamageCorrection(target, entity);
            break;
        case 'starry_map:abyss_whale.detection':
            passive_component.AbysssWhaleDetectionWasHit(entity);
            break;
        case 'starry_map:dragon.tyrannosaurus_rex':
            passive_component.tyrannosaurusRexAttack(target);
            break;
        default: break;
    }
}
 ...
 * @param {server.Entity} entity - 发起攻击的实体
 */
export function PlayersUnderAttack(target, entity) {
    // 检测是否为 玩家类型
    if (!(target instanceof server.Player))
        return;
    /**
 ...
 * @param {server.Entity} entity - 发起攻击的实体
 */
export function PlayersLaunchAttacks(target, source, entity) {
    // 检测是否为 玩家类型
 ...
 */
export function createRewardsAfterDeath(target, self) {
    /**
     * * 死亡的实体是否为< 地方传奇 >
     */
    ...
    if (enable && !blacklist.has(state.self_rune))
        opal.TrySpawnItem(dimension, new server.ItemStack('starry_map:' + state.self_rune.split('_')[1] + '_energy'), target.location);
    // 给与击杀者经验
    if (experience <= table.max_experience * table.experience_improve)
        self.setDynamicProperty('entity:experience', experience + Math.floor((intel ?? 1) / 10) + 1);
    // 验证 是否满足发放条件
    if (!intel || !opal.IsEnable(intel))
        return;
 ...
 */
export async function FunctionsPerformedAfterDeath(entity, source) {
    // 根据实体类型进行判断并执行相应的函数
    switch (entity.typeId) {
        //* 渊鲸侦测者
        case 'starry_map:abyss_whale.detection':
            passive_component.AbyssWhaleDetectionDie(entity, source);
            break;
        //* 渊鲸执行者
        case 'starry_map:abyss_whale.execute':
            passive_component.AbyssWhaleExecuteDie(entity, source);
            break;
        //* 野蜂维系者
        case 'starry_map:wild_bee.support':
            passive_component.WaspClusterCrash(entity);
            break;
        //* 玩家
        case 'minecraft:player':
            passive_component.PlayerDie(entity);
            break;
    ...
    if (entity.typeId.split(':')[1].split('.')[0] == 'guide' && !entity.getDynamicProperty('entity:contract_user')) {
        // 验证 控制器冷却是否完成
        if (!opal.TriggerControl('君王暴龙-召唤限速', new opal.Vector(0, 0, 0), 200))
            return;
        // 验证 实体类型
        ...
        let thunderIndex = 0;
        // 循环遍历查询范围
        for (let axleX = -queryScope; axleX <= queryScope; axleX += 2)
            for (let axleZ = -queryScope; axleZ <= queryScope; axleZ += 2) {
                /**
                 * * 计算闪电坐标
                 */
