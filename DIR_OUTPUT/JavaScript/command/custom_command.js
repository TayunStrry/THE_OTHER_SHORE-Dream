...
components.set({
    ...
    // 判断触发器是否已禁用
    if (!opal.TriggerControl('使用自定义指令添加牵引力', self, 100))
    ...
    server.system.run(() => {
        ...
        targets.forEach(async (entity) => {
            ...
            if (entity instanceof server.Player)
                entity.onScreenDisplay.setActionBar([opal.translate(self), ' <- 正在牵引 -> ', opal.translate(entity)]);
            // 等待 20 帧
...
components.set({
    ...
    server.system.run(() => {
        ...
        // 卸载被选中的实体
        if (player)
            opal.UnloadInventoryAndPackage(entity, player, item, customName || '§l§q概念封装§r - ', customLore ? [customLore] : []);
        // 删除 控制器物品
        if (container)
            opal.DeleteItemStack(container, new server.ItemStack('starry_map:mechanized_operation'));
    });
    // 显示运行成功的消息
...
components.set({
    ...
    server.system.run(() => {
         ...
         * 合并玩家当前属性与新增伤害值
         */
        const property = opal.MergeProperty(opal.GetProperty(player), { raise_basic_attack: damage, 'self_rune': type });
        /**
         * 判断是否触发暴击
         */
        ...
        // 遍历实体队列并加入按钮
        if (queue.length >= 1)
            queue.forEach(entity => display.button(opal.DistanceAndName(entity, Distance(entity))));
        else
