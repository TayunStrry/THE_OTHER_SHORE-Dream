 ...
 * 数学拓展模块
 */
import { Vector, CalculateMedian, CalculateModes, AnalysisWeight, RandomFloor, RandomFloat, Random, Clamp, IsEnable, QueryFoothold, QueryEntityFoothold } from './maths';
 ...
 * 计划执行模块
 */
import { Control, Template, PathExecute, MistySeaFissure, NumberID, BriefID, UUID } from './plan';
export { Control, Template, PathExecute, MistySeaFissure, NumberID, BriefID, UUID };
/*
 * 实例创建模块
 */
import { TrySetPermutation, TrySpawnParticle, TrySpawnItem, TryFillBlocks, TrySpawnEntity, SetFreePointer, TryProcessBlocksInVolume } from './create'
 ...
 * 月华百科模块
 */
import { ReplyMessages, material, manageChatResponses, windowedRetriever, lexiconWindowedInterface, isPlayerAuthorized, lexiconInterface } from './lexicon_v5';
 ...
 * 元素攻击模块
 */
import { CreateProperty, SetProperty, AlterProperty, MergeProperty, GetProperty, CreateEmptyProperty, ElementalAttack, IsErupt, BackoffByDistance } from './rune_attack';
export { CreateProperty, SetProperty, AlterProperty, MergeProperty, GetProperty, CreateEmptyProperty, ElementalAttack, IsErupt, BackoffByDistance };
/*
 * 实体重载模块
 */
import { UnloadInventoryAndPackageInPlace, UnloadInventoryAndDestroy, UnloadInventoryAndPackage, SummonEntityWithData } from './reload';
export { UnloadInventoryAndPackageInPlace, UnloadInventoryAndDestroy, UnloadInventoryAndPackage, SummonEntityWithData };
/*
 * 区块处理模块
 */
import { DisplayChunkBoundary, RealmPropertyName, AlterEnergy, QueryEnergy, CreateMineralVein, QueryMineralVein, ExpendMineralVein } from './chunk';
 ...
 * 触发控制模块
 */
import { TriggerControl, ObtainWaitTime } from './control';
export { TriggerControl, ObtainWaitTime };
/*
 * 物品处理模块
 */
import { CreateItemFromStackData, DisplaceItemStack, SetDurability, AlterDurability, DeleteItemStack, ConsumeItemStack, CheckItemHasAmount, OrganizeItemStacks, CheckItemStack, SearchContainers } from './parse_item';
 ...
 * 信息处理模块
 */
import { TemplateMatcher, CleanMessageArray, JsonToUnicode } from './parse_text';
export { TemplateMatcher, CleanMessageArray, JsonToUnicode };
/*
 * 实体处理模块
 */
import { GetContractRoles, EntitysSort, GetPartner } from './parse_entity';
export { GetContractRoles, EntitysSort, GetPartner };
/*
 * 信息处理模块
 */
import { GetTargetIntel, DistanceAndName, ThrowErrorIfPermitted, IntelMessage, DisplayFloatingText, AlterMessageNotify, ErrorMessage, NumberParticleDisplay } from './intel';
 ...
 * * 水花粒子效果触发器
 */
function SprayParticleTrigger(dimension: server.Dimension, location: server.Vector3, size: number = 1) {
	/**
	 * * 参数数组
	 */
 ...
 */
function ExecuteAnalysisRealm(player: server.Player, text: string, after: (player: server.Player, args: type.INTENTION_REALM_AFTER) => void) {
	/**
	 * 获取领域属性标识符
	 */
	 ...
	 * 定义窗口界面标题
	 */
	const title: server.RawMessage = { text: "§9<§l " + text + " §r§9>" };
	/**
	 * 创建窗口界面表单对象
	 */
	...
	// 根据领域属性数量添加按钮
	if (entry.length >= 1) entry.forEach((key, index) => display.button('§l§9基准坐标 : §5' + mapping(key) + '\n§9属性值 : §r§u' + state[index]));
 ...
 */
function ExecuteAlterEnergy(player: server.Player, args: type.INTENTION_REALM_AFTER) {
	/**
	 * 定义表单标题
	 */
	 ...
	 * 输入框提示信息
	 */
	const text: server.RawMessage = { text: "请输入新的星尘能量值, 修改需谨慎以避免故障" };
	/**
	 * 预填充的文本
	 */
	const plan: server.RawMessage = { text: args.state[args.option.selection ?? 0]?.toString() };
	/**
	 * 创建表单对象, 并添加能量值输入框
	 */
 ...
 * @param {INTENTION_REALM_AFTER} args - 表单返回的参数
 */
function AlterMineral(player: server.Player, args: type.INTENTION_REALM_AFTER) {
	/**
	 * 定义表单标题
	 */
	 ...
	 * 输入框提示信息
	 */
	const text: server.RawMessage = { text: "请输入新的矿物参数, 修改需谨慎以避免故障" };
	/**
	 * 解析当前领域属性
	 */
 ...
 */
function AnalysisAlterEnergy(player: server.Player): server.RawMessage {
	// 权限验证
 ...
 * @returns {server.RawMessage} - 操作结果的反馈消息
 */
function AnalysisAlterMineral(player: server.Player): server.RawMessage {
	// 权限验证
	if (!isPlayerAuthorized(player)) return ReplyMessages.power_lack;
	// 调用领域分析函数, 调整矿物值
