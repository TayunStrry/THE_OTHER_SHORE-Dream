...
export class BridgeProtocol {
	...
	/**
	 * * 批量转移方法, 将对象数组中的函数提取出来, 并存储到函数列表中, 同时可以传入对应的描述信息数组
	 *
	 * @param {Record<string, any>} [functionsObj] - 包含各种类型的函数的对象, 其中可能包含函数和类
	 */
	public batchExportFunction(functionsObj: Record<string, any>) {
		/**
		 * 从对象数组中筛选出函数, 并排除类
		 *
		 * @type {ExportFunctionType[]}
		 */
		const extractFunctions: ExportFunctionType[] = Object.values(functionsObj).filter(value => typeof value === 'function' && !/^\s*class\s+/.test(Function.prototype.toString.call(value))) as ExportFunctionType[];
		// 将提取到的函数添加到函数列表中
		extractFunctions.forEach(value => this.functionList.set(value.name, value));
	};
	/**
	 * * 显示帮助信息的方法
	 *
