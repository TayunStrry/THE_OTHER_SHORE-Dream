...
components.set("cit:dynamic_state", {
    ...
    onPlayerInteract(event) {
        ...
        if (variant === 1 || variant === 2) {
            ...
            if (typeof baseCurrentState !== "number")
                return;
            /**
             * 调用自定义函数获取基础方块的最大状态数
             */
            ...
            // 如果上方方块存在，则更新其动态状态
            if (topBlock)
                create.TrySetPermutation(topBlock, "cit:dynamic_state", baseNewState);
        }
    ...
    beforeOnPlayerPlace(event) {
        ...
        if (aboveBlock.isValid && aboveBlock.isAir) {
            ...
            server.system.run(() => {
                ...
                create.TrySetPermutation(aboveBlock, "minecraft:block_face", permutationToPlace.getState("minecraft:block_face"));
            });
        ...
        // 若上方和下方方块都不满足条件
            ...
            for (let index = 0; index < space.length; index++) {
                ...
                if (!target || !target.isAir || !targetAbove || !targetAbove.isAir)
                    continue;
                // 使用 server.system.run 确保在合适的时机执行方块设置操作
                server.system.run(() => {
                    /**
