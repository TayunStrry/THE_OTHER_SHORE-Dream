 ...
 */
export function getLegionBase(entities, legionType) {
 ...
 */
export function legionBaseUnderAttack(entity) {
    // 触发器控制
    ...
    dimension.getEntities().forEach(target => {
        ...
        if (!familyComponent)
            return;
        // 销毁现存的军团成员
        if (target.typeId.split(':')[1] !== 'player' && target.typeId.split(':')[1] !== 'legion_base')
            return target.remove();
        // 销毁现存的怪物
...
export function tickEvent() {
    ...
    // ! 判断是否应该终止战争
    if (CombatAttribute.shouldTheWarBeTerminated == 1)
    ...
    const planEvent = (plan) => {
        try {
             ...
             * 尝试生成军团成员实体
             */
            const targetEntity = TrySpawnEntity(dimension, mappingEntityTypeID, adjustSpawnLocation);
            // 判断实体是否生成错误
 ...
 * 更新士兵行为的函数
 */
export async function updateSoldiersBehavior() {
    /**
     * 获取当前世界中的第一个玩家
     */
    let targetPlayer = globalState.players[0];
 ...
 * @param self 执行攻击的实体
 */
export function attackEvent(self) {
    // 检查实体是否有效
    ...
    if (!families)
        return;
    if (families.some(family => family.includes('灰色阵营')))
        return;
    if (families.some(family => family.includes('LegionBase')))
        return;
    // 获取阵营标签并添加空值检查
