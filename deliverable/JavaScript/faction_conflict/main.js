...
function tickEvent() {
    ...
    const planEvent = (plan) => {
        try {
             ...
             * 尝试生成军团成员实体
             */
            const targetEntity = TrySpawnEntity(dimension, mappingEntityTypeID, plan.legionIndexValue == 1 ? blueLegionBase.location : redLegionBase.location);
            // 判断实体是否生成错误
            if (targetEntity instanceof Error)
                return console.error(targetEntity.message, targetEntity.stack);
            // 为生成的实体赋予名称
...
function readGlobalVariables() {
    ...
    if (publicLegionIndexValue !== 1 && publicLegionIndexValue !== 2)
        return false;
    // 将全局变量加入计划表
 ...
 */
function parameterDisplay(allowDisplay) {
    // 如果常驻显示和允许显示都不为真，则不执行后续代码
    if (!continuousDisplay && !allowDisplay)
        return;
    // 遍历全体玩家并显示全局公开参数
...
function selectEntityInServerUI(player, legionIndexValue) {
    ...
    if (legionIndexValue == 1)
        memberDirectory.forEach(type => window.button(translate(type, 'entity'), 'textures/blue_legion_base_config'));
    if (legionIndexValue == 2)
        memberDirectory.forEach(type => window.button(translate(type, 'entity'), 'textures/red_legion_base_config'));
    // 显示表单窗口
    window.show(player).then(response => {
        // 如果玩家取消了窗口界面, 则不执行后续代码
        if (response.canceled || response.selection == undefined)
 ...
 */
function setPublicProperty(player) {
    /**
    ...
    window.show(player).then(response => {
        ...
        publicEntityNameValue = response.formValues[3] || ''; // 实体名称值
    });
    // 开启持续显示模式
...
function legionBaseUnderAttack(entity) {
    ...
    dimension.getEntities().forEach(target => {
        ...
        if (!familyComponent)
            return;
        // 销毁现存的军团成员
        if ((target.hasTag('red_legion') || target.hasTag('blue_legion')) && target.typeId.split(':')[1] !== 'legion_base')
            return target.remove();
        // 销毁现存的怪物
