 ...
 */
export function selectLegionInServerUI(player) {
    /**
     * 新建一个 Action 表单窗口
     */
    ...
    window.show(player).then(response => {
        ...
        if (response.selection == 2)
            return CombatAttribute.shouldTheWarBeTerminated = 1;
        /**
         * 获取玩家选择的阵营索引
         */
 ...
 */
export function selectEntityInServerUI(player, legionIndexValue) {
    /**
     * 新建一个 Action 表单窗口
     */
    const window = new serverUI.ActionFormData();
    ...
    if (legionIndexValue == 1)
        globalState.memberDirectory.forEach(type => window.button(translate(type, 'entity'), 'textures/blue_legion_base_config'));
    if (legionIndexValue == 2)
        globalState.memberDirectory.forEach(type => window.button(translate(type, 'entity'), 'textures/red_legion_base_config'));
    // 显示表单窗口
    window.show(player).then(response => {
        // 如果玩家取消了窗口界面, 则不执行后续代码
        if (response.canceled || response.selection == undefined)
 ...
 */
export function writeEntityNameInServerUI(player, legionIndexValue, entityIndexValue) {
    /**
     * 新建一个 Modal 表单窗口
     */
 ...
 */
export function setPublicProperty(player) {
    /**
    ...
    window.show(player).then(response => {
        ...
        CombatAttribute.publicEntityNameValue = response.formValues[3] || ''; // 实体名称值
    });
    // 开启持续显示模式
