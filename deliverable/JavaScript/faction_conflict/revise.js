...
function rebindAggroTarget() {
    ...
    mojang.world.events.tick.subscribe(() => {
        ...
        // 检测军团的基地是否全部存在
        if (!redBase || !blueBase)
            return console.error("未找到完整的军团基地");
        // 为红方军团设置蓝方基地为目标
 ...
 * @param tag - 阵营标签
 */
function newSetAggroTarget0(entities, targetBase, tag) {
    entities.forEach(self => {
...
;
function setAggroTarget(entities, targetBase, tag) {
    entities.forEach(self => {
        // 排除无效实体
        if (!self.hasTag(tag) || self.id.split(':')[1] === 'legion_base')
            return;
...
;
function newSetAggroTarget(entities, targetBase, tag) {
    entities.forEach(self => {
        // 排除无效实体
        if (!self.hasTag(tag) || self.id.split(':')[1] === 'legion_base')
            return;
        ...
        if (!self.hasTag('approaching_enemy_camps')) {
            // 添加标签
            if (selfLocation.distance(targetBase.location) <= 5)
                return self.addTag('approaching_enemy_camps');
            /**
             * 拷贝目标位置
             */
            const targetLocation = Vector.copy(targetBase.location);
