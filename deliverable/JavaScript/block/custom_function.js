 ...
 * * 伺服驱动 前处理事件
 */
export function servoDriveBeforeEvent(analysis, offsetY = -1) {
    /** 方块信息解构 */
    ...
    if (typeof condition !== 'number')
        return;
    /** 查询方块标签的辅助函数 */
    const hasTag = (offset, tag) => block.offset(offset)?.getTags().includes(tag) ?? false;
    /** 方向配置映射 */
    ...
    if (condition >= 1 && condition <= 4) {
        ...
        if (hasTag(config.offset, 'tags:magic_cable.series') && !hasTag(config.checkOffset, 'tags:magic_cable.series')) {
            ...
            switch (config.direction) {
                case 'X+':
                    block.setPermutation(state.withState('STATE:direction', 1));
                    break;
                case 'X-':
                    block.setPermutation(state.withState('STATE:direction', 2));
                    break;
                case 'Z+':
                    block.setPermutation(state.withState('STATE:direction', 3));
                    break;
                case 'Z-':
                    block.setPermutation(state.withState('STATE:direction', 4));
                    break;
            }
        }
    }
 ...
 * * 伺服驱动 后处理事件
 */
export function servoDriveAfterEvent(analysis, multiplier = 1, callback) {
    /** 方块信息解构 */
    const { block, state, dimension } = analysis;
    /** 方块状态值 */
    const direction = state.getState('STATE:direction');
    /** 当前状态值 */
    ...
    /** 方向配置项 */
    const dirConfig = directionVectors[direction];
    // 检查方向配置是否存在
    if (!dirConfig)
        return;
    // 执行功能逻辑
    for (let index = 0; index <= 5; index++) {
        ...
        const [anchor_0, anchor_1, anchor_2, anchor_3] = [
            ...
            opal.Vector.toString(block.offset(dirConfig.move), { delimiter: ' ' }),
            opal.Vector.toString(block.offset({ x: dirConfig.offset.x, y: index * multiplier, z: dirConfig.offset.z }), { delimiter: ' ' }),
            opal.Vector.toString(block.offset({ x: 0, y: index * multiplier, z: 0 }), { delimiter: ' ' })
        ];
        // 执行回调函数
