 ...
 * * 物品抽取事件
 */
export function extractEvent(player, entity, block, container) {
    /**
     * * 事件
     *
 ...
 * * 物品注入事件
 */
export function injectEvent(player, entity, block, container) {
    /**
     * * 事件
     *
     * @param {server.Container} input 容器
...
export function containerSorting(player, block) {
    ...
    ;
    // 重新 放入 方块容器
    for (let item of opal.OrganizeItemStacks(items))
 ...
 */
export function signPresets(input) {
    /**
     * * 获取 道标名称
     */
 ...
 * * 执行 相对传送
 */
export function renRelative(player, option) {
    //检测玩家是否退出窗口
    if (option.canceled)
        return;
    /**
 ...
 * * 执行 随机传送
 */
export function renRandom(player, option) {
    //检测玩家是否退出窗口
    if (option.canceled)
        return;
    /**
     ...
     * * 获取锚点位置
     */
    const anchor = opal.QueryFoothold({ location: location, dimension: player.dimension }, parseInt(option.formValues[3]), -60, 300);
    // 传送玩家
 ...
 * * 执行 道标传送
 */
export function renRoadSign(player, option, input) {
    //检测玩家是否退出窗口
 ...
 */
export function sourceEnergy(player) {
    /**
     * 获取所有引导文本的标题
     *
     * @type {string[]}
     */
    const keys = Array.from(prompt.keys());
    /**
     * 获取所有引导文本的内容
     *
     * @type {server.RawMessage[][]}
     */
    const value = Array.from(prompt.values()).map(item => item.map(info => info.refer));
    /**
     * 定义了 窗口界面 的 标题
     *
     * @type {server.RawMessage}
 ...
 * * 锚点绑定
 */
export function bindingAnchor(player) {
    /**
     * * 设定查询参数
     */
    const options = {
 ...
 * * 锚点召集
 */
export function musterAnchor(player) {
    /**
     * * 设定查询参数
     */
    const options = {
 ...
 * * 锚点移除
 */
export function deleteAnchor(player) {
    /**
     * * 设定查询参数
     */
    const options = {
 ...
 * * 锚点传送
 */
export function arrivalAnchor(player) {
    /**
     * * 设定查询参数
     */
    const options = {
...
export function obtainBlock(player, container, block) {
     ...
     * * 方块的附加信息
     */
    const message = { rawtext: [{ text: '获取到: ' }, opal.translate(block)] };
    // 在背包中添加方块物品
 ...
 * * 获取 伤害提升
 */
export function increaseDamage(player, container, single = true) {
    // 遍历 背包容器
    for (let index = 0; index < container.size; index++) {
        /**
         * * 获取 物品对象
 ...
 * * 魔晶弹弓 - 潜行时
 */
export function bowSneaking(object) {
    // 创建 路径包
 ...
 * 判断是否满足连锁挖掘条件
 */
export function canTriggerChainMining(player, itemStack, type) {
    // 判断标识符与触发器是否允许触发
    if (!type || !opal.TriggerControl('区块连锁', player, 20))
        return false;
    // 匹配玩家状态与玩家设置是否匹配1
    switch (type) {
 ...
 * 检查耐久度是否足够
 */
export function checkDurability(durability, waste, player, itemStack) {
    /**
     * 耐久度差值
     */
 ...
 * 添加挖掘疲劳和饥饿效果
 */
export function applyMiningEffects(player) {
 ...
 */
export function executeMiningPath(dimension, blockID, location, range, depth, height, player) {
    /**
    ...
    const moveEvent = (args) => {
        ...
        if (getBlock?.typeId === blockID) {
             ...
             */
            const itemOptions = { location: args.location, type: "minecraft:item", maxDistance: 4 };
            /**
             * 用于查询经验球实体的选项。
             *
             * 它定义了在当前挖掘点附近搜索经验球的范围。
             */
            const expOptions = { location: args.location, type: "minecraft:xp_orb", maxDistance: 4 };
            /**
...
function shieldReflexDamage(player, item) {
    ...
    // 生成 粒子效果
    if (getData.self_rune !== type.RUNE_ENUM.void)
        opal.TrySpawnParticle(player.dimension, `constant:impact_${getData.self_rune}`, player.location);
    if (getData.self_rune === type.RUNE_ENUM.void)
        opal.TrySpawnParticle(player.dimension, 'minecraft:sonic_explosion', player.location);
    // 更新 物品冷却
    item.getComponent('minecraft:cooldown')?.startCooldown(player);
}
;
 ...
 * * 魔晶钥匙
 */
export async function magicCrystalKey(player, item, container, block) {
    /**
     * * 方块白名单
     */
 ...
 * * 生成 初始角色
 */
export function initialSummoning(player) {
    /**
     * * 定义 队列信息
     */
 ...
 */
export function randomCallRole(player, type) {
    /**
     * * 定义 坐标基准点
     */
