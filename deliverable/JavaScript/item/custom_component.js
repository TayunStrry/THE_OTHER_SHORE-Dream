...
components.set(componentPrefix + 'blank_industrial_blueprint', {
    ...
    onMineBlock(source) {
        ...
        const transformation = (option) => {
            ...
            // 检测 区域体积是否过小
            if (Math.abs(vector.x) + Math.abs(vector.y) + Math.abs(vector.z) <= 9)
                return player.sendMessage('§c§l 当区域体积过小, 无法创建结构!');
            // 创建 结构
...
components.set(componentPrefix + 'industrial_blueprint', {
    onUseOn(source) {
        ...
        if (!player.isSneaking)
            return;
        // 消耗 能量 并确认 能量 是否充足
        if (opal.ControlStardustEnergy(block)[0] <= volume * 1000)
