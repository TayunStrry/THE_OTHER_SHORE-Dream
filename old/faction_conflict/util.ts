 ...
 */
export function readGlobalVariables(): boolean {
    // 如果设定的阵营值是非法值, 则不加入计划表
    if (CombatAttribute.publicLegionIndexValue !== 1 && CombatAttribute.publicLegionIndexValue !== 2) return false;
    // 将全局变量加入计划表
 ...
 */
export function parameterDisplay(allowDisplay: boolean): void {
    // 如果常驻显示和允许显示都不为真，则不执行后续代码
    if (!CombatAttribute.continuousDisplay && !allowDisplay) return;
    // 遍历全体玩家并显示全局公开参数
